...
echo "######################################################################################################"
 yum -y install policycoreutils-python vim epel-release
...
echo "######################################################################################################"
yum -y install rsyslog httpd nagios-plugins-all mariadb-server mariadb-libs mariadb icinga2 icinga2-doc icinga2-selinux icinga2-ido-mysql icingaweb2 icingacli php-ZendFramework php-ZendFramework-Db-Adapter-Pdo-Pgsql php-ZendFramework-Db-Adapter-Pdo-Mysql php-ldap
...
mysql -uroot -prootpass -e "CREATE DATABASE icinga2"
mysql -uroot -prootpass -e "GRANT ALL PRIVILEGES ON icinga2.* TO 'icinga2'@'localhost' IDENTIFIED BY 'IcingaPass'; FLUSH PRIVILEGES;"
mysql -uroot -prootpass icinga2 < /usr/share/icinga2-ido-mysql/schema/mysql.sql
mysql -uroot -prootpass -e "CREATE DATABASE IF NOT EXISTS icingaweb2 ; GRANT ALL ON icingaweb2.* TO icingaweb2@localhost IDENTIFIED BY 'icingaweb2';"
